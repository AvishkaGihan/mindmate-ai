...
export class AuthService {
  ...
  public async loginWithFirebase(idToken: string): Promise<AuthResult> {
    try {
      ...
      // Handle Firebase specific errors or generic failures
  ...
  private signAccessToken(user: IUser): string {
    // We assume user._id exists because it's retrieved from DB
    return jwt.sign(
      {
        userId: user._id,
        email: user.email,
      ...
      config.auth.jwtSecret as string | Buffer,
  ...
  private signRefreshToken(user: IUser): string {
    return jwt.sign(
      { userId: user._id },
      config.auth.jwtRefreshSecret as string | Buffer,
      {
